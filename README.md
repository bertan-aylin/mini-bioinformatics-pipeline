...
#Input Data
The pipeline takes a FASTQ file as input. FASTQ files contain sequencing reads with their quality scores for each base. These quality scores indicate the reliability of base calls produced during sequencing. The pipeline reads the FASTQ file and extracts read-level information required for quality assessment. 
#Workflow Steps
FASTQ file
↓
...
#Environment
The pipeline is generated using Snakemake and runs within a Conda environment defined in the environment.yml file. This environment specifies necessary softwar dependencies and ensures that the analysis can be reproduced across different systems.
#How To Run
To run the pipeline:
